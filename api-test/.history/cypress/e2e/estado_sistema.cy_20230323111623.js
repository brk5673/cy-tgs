...
describe('Test cases de modulo Estado de Sistema', () => { // corregir describe
  ...
  it.only('us851 - Validacion Agregar nuevo estado', () => {
    ...
    cy.contains('Listar').click()
    cy.get('.MuiTableCell-root.MuiTableCell-body.MuiTableCell-sizeSmall').should('exist') // validar contenido de tabla
    cy.get('#root > div > div > main > div > div.MuiGrid-root.MuiGrid-container.MuiGrid-grid-xs-12 > div > div.tabs_swipper > div > div > div > div.MuiGrid-root.table_paper_addButton.MuiGrid-container.MuiGrid-item.MuiGrid-grid-xs-12 > div > button:nth-child(2) > span.MuiButton-label').click() // click en el boton agregar nuevo
