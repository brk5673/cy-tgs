...
describe('Test cases de modulo Estado de Sistema', () => { // corregir describe
  ...
  it.only('us848 - Validacion eliminar estado de grilla', () => {
    ...
    cy.contains('Listar').click()
    cy.get('.MuiTableCell-root.MuiTableCell-body.MuiTableCell-sizeSmall').should('exist') // validar contenido de tabla
    cy.get('#root > div.jss1 > div > main > div > div.MuiGrid-root.MuiGrid-container.MuiGrid-grid-xs-12 > div > div.tabs_swipper > div > div > div > div.table_paper_list > div > table > tbody > tr:nth-child(1) > td:nth-child(9) > button').click() // click en el icono para eliminar fila
  })
