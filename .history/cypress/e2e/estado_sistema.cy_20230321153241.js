...
describe('Test cases de modulo Estado de Sistema', () => { // corregir describe
  ...
  it.only('us833 - Validacion exportar PDF', () => {
    cy.visit('http://10.1.11.237:8080/etgs/spac/control/estadodelsistema')
      cy.get('#navPath').should('contain.text', 'Estado del Sistema', {matchCase: false})
    cy.get('[name="inicial"]').clear().type('16032023')
    cy.get('[name="final"]').clear().type('16032023')
    ...
    cy.task('waitForDownload') //espernado que complete la descarga
    cy.task('getLasDownload').then((filename) => {
      expect(filename).to.match(/\.pdf$/) //verifica que el archivo descargado tenga extension .pdf

      const content = fs.readFileSync('~/Downloads/${filename}') // lee el contenido del archivo descargado
      PdfParse(content).then((data) => {
        expect(data.text).to.contain('Estados del sistema')
      })
    })
  })
