...
describe('Test cases de modulo Estado de Sistema', () => { // corregir describe
  ...
  //dirigirse al punto configContactos por UI
    ...
    cy.get('table').contains('th', 'Sin MAPO').should('exist')
    cy.get('table').contains('th', 'Con MAPO').should('exist')
    cy.get('table').contains('th', 'Defitit de Inyeccion').should('exist')
  ...
  it('us1123 - Validacion filtros <Gasoducto/Entidad_Legal/Contacto>', () => {
    ...
    cy.get('input[name="nombreGasoducto"]').type('GASODUCTO 1')
    cy.get('input[name="nombreEntidadLegal"]').type('ENTIDAD LEGAL 1')
    cy.get('input[name="nombre"]').type('CONTACTO 1')
