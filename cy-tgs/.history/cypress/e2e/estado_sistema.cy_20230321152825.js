...
describe('Test cases de modulo Estado de Sistema', () => { // corregir describe
  ...
  it.only('us833 - Validacion exportar PDF', () => {
    ...
    cy.task('getLasDownload').then((estadoDelSistema) => {
      expect(estadoDelSistema).to.match(/\.pdf$/) //verifica que el archivo descargado tenga extension .pdf

      const content = fs.readFileSync('~/Downloads/${estadoDelSistema}')
