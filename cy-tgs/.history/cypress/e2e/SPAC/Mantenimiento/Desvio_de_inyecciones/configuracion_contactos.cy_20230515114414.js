...
describe('Test cases de modulo <SPAC/Mantenimiento/Desvio de inyecciones/Configuracion de contactos>', () => { // corregir describe
  ...
  it.only('us1140 - validar exportar reportes', () => {
    ...
    cy.get('.MuiTableCell-root.MuiTableCell-body.MuiTableCell-sizeSmall').should('exist') // validate table
    cy.get('div.HeaderSvgCustomIcon.PdfIcon').should('be.visible').click() // click on pdf button
    cy.wait(3000) // Espera unos segundos para que se complete la descarga

cy.readFile('downloads/document.pdf').should('exist') // Verifica que el archivo descargado exista
